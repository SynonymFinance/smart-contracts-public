...
import {
    ...
    RelayerCannotReceivePayment,
    TargetChainNotSupported,
    ParamAlwaysEmptyInSend,
...
abstract contract WormholeRelayerSend is WormholeRelayerBase, IWormholeRelayerSend {
    ...
    */
     ...
     */
    function getDefaultDeliveryProvider() external pure override returns (address) {
        return address(0);
    }
     ...
     */
    function quoteSolanaDeliveryPrice() public view override returns (LocalNative nativePriceQuote) {
        uint256 deliveryPriceSol = WormholeRelayerStorage.getRoutingCostConfig().solanaDeliveryPrice;

        ISynonymPriceOracle.Price memory wormholeSolPrice = priceOracle.getPrice(solAssetId);
        ISynonymPriceOracle.Price memory wethPrice = priceOracle.getPrice(wethAssetId);

        uint256 deliveryPriceWeth =
            (deliveryPriceSol * wormholeSolPrice.price * WETH_DECIMALS) /
            (wethPrice.price * SOL_DECIMALS);
     ...
     */
    function getRelayerReward() public view override returns (LocalNative nativePriceQuote) {
        uint256 relayerReward = WormholeRelayerStorage.getRoutingCostConfig().relayerReward;
