...
import {
    ReentrantDelivery,
    VaaKey,
    MsgValueNotEnoughForDeliveryCosts,
    IWormholeRelayerBase,
    RelayerPaymentTooSmall
...
abstract contract WormholeRelayerBase is IWormholeRelayerBase {
    ...
    ISynonymPriceOracle internal immutable priceOracle;
    bytes32 internal immutable solAssetId;
    bytes32 internal immutable wethAssetId;

    modifier nonReentrant() {
        // Reentrancy guard
        WormholeRelayerStorage.ReentrancyGuardState storage reentrancyState = WormholeRelayerStorage.getReentrancyGuardState();
    ...
    constructor(address _wormhole, address _priceOracle, bytes32 _solAssetId, bytes32 _wethAssetId) {
    ...
    function getRegisteredWormholeRelayerContract(uint16 _chainId) public view returns (bytes32) {
        return WormholeRelayerStorage.getRegisteredWormholeRelayersState().registeredWormholeRelayers[_chainId];
    }

    function deliveryAttempted(bytes32 deliveryHash) public view returns (bool attempted) {
        return WormholeRelayerStorage.getDeliverySuccessState().deliverySuccessBlock[deliveryHash] != 0 ||
            WormholeRelayerStorage.getDeliveryFailureState().deliveryFailureBlock[deliveryHash] != 0;
    }

    function deliverySuccessBlock(bytes32 deliveryHash) public view returns (uint256 blockNumber) {
        return WormholeRelayerStorage.getDeliverySuccessState().deliverySuccessBlock[deliveryHash];
    }

    function deliveryFailureBlock(bytes32 deliveryHash) public view returns (uint256 blockNumber) {
        return WormholeRelayerStorage.getDeliveryFailureState().deliveryFailureBlock[deliveryHash];
    }
    ...
    function checkMsgValue(
        ...
        if (msgValue() < solanaDeliveryPrice + wormholeMessageFee + relayerReward) {
            revert MsgValueNotEnoughForDeliveryCosts(msgValue(), solanaDeliveryPrice + wormholeMessageFee + relayerReward);
        }
    }
    ...
    function publishAndPay(
        ...
        LocalNative relayerPaymentForSolanaDelivery = LocalNative.wrap(msg.value) - relayerReward - wormholeMessageFee;
        ...
        if(relayerPaymentForSolanaDelivery < solanaDeliveryPrice) {
            revert RelayerPaymentTooSmall(solanaDeliveryPrice, relayerPaymentForSolanaDelivery);
        }
